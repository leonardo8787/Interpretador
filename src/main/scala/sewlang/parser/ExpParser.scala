...
object ExpParser {
  ...
  def parse(sexp: SExpr): Exp = sexp match {
    ...
    case SList(List(SSym("<="), exp1, exp2)) => LessOrEqualThanExp(parse(exp1), parse(exp2)) // (<= exp1 exp2)
    case SList(List(SSym(">"), exp1, exp2)) => LargerThanExp(parse(exp1), parse(exp2)) // (> exp1 exp2)
    case SList(List(SSym(">="), exp1, exp2)) => LargerOrEqualThanExp(parse(exp1), parse(exp2)) // (>= exp1 exp2)
